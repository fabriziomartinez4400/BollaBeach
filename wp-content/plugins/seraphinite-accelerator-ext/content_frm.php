...
' );
	...
	if( Gen::GetArrField( $settCp, array( 'wcs' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );

		if( HtmlNd::FirstOfChildren( $xpath -> query( './/script[contains(text(),"woocs_default_currency")]' ) ) )
		{
			...
			HtmlNd::SetValFromContent( $itemScript, "(function(d,a){a.woocs_redirect_orig=a.woocs_redirect;a.woocs_redirect=function(b){var e=a.woocs_default_currency.name==b?\"\":b,c=new Date;c.setTime(c.getTime()+31536E6);d.cookie=\"wccs_changed_currency=\"+e+\";path=/;SameSite=Lax;expires=\"+c.toUTCString();a.woocs_redirect_orig(b)}})(document,window)" );
			$ctxProcess[ 'ndBody' ] -> appendChild( $itemScript );
		}
	}
...
function _ProcessCont_Cp_swiper_AdjustItem( $item, $itemSwpr, $aPrm, $ctx, &$ctxProcess, $doc, $xpath )
{
	...
	$cssClassPagination = ($aPrm[ 'cssClassPagination' ]??'swiper-pagination');
	...
	foreach( $aPrm[ 'breakpoints' ] as $dim )
	{
		...
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > * > *:nth-child(n+' . ( string )( $nItems + 1 ) . ') { visibility: hidden !important; }';
	...
	{
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > * { flex-direction: column; }';
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > * > * { margin-bottom: var(--lzl-swpr-sps) !important; height: calc((var(--lzl-swpr-sz) - var(--lzl-swpr-sps) * (var(--lzl-swpr-n) - 1)) / var(--lzl-swpr-n)) !important; width1: 100% !important; }';
	}
	else
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > * > * { margin-right: var(--lzl-swpr-sps) !important; width: calc((var(--lzl-swpr-sz) - var(--lzl-swpr-sps) * (var(--lzl-swpr-n) - 1)) / var(--lzl-swpr-n)) !important; height1: 100% !important; }';
	...
	if( $paginationType && ( $itemPagination = HtmlNd::FirstOfChildren( _Cp_XPathCssQuery( $ctx, $xpath, '.' . $cssClassPagination, $item ) ) ) )
	{
		HtmlNd::AddRemoveAttrClass( $itemPagination, array( $cssClassPagination . '-' . $paginationType, $cssClassPagination . '-' . $direction ) );
...
function _ProcessCont_Cp_slick_PrepareCont( $ctx, $doc, $xpath, $itemSlides, $classSlide, $bInsideCtls = false, $cbSlide = null )
{
	if( !$itemSlides )
		return( null );
...
function _ProcessCont_Cp_slckGen( $a, $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	foreach( $a as $ai )
	{
		...
		foreach( _Cp_XPathCssQuery( $ctx, $xpath, $selCss ) as $item )
		{
			...
			$sld = _ProcessCont_Cp_slick_PrepareCont( $ctx, $doc, $xpath, $item, null, Gen::GetArrField( $ai, array( 'dots' ), false ) || Gen::GetArrField( $ai, array( 'arrows' ), true ),
			...
			$aDotsClass = ( array )Gen::GetArrField( $ai, array( 'dotsClass' ), 'slick-dots', '', false, false );
			$cbCustomPaging = Gen::GetArrField( $ai, array( 'customPaging' ) );
			if( !$cbCustomPaging )
				$cbCustomPaging = function( $sld, $i ) { return( Ui::Tag( 'li', sprintf( '<button type="button">%s</button>', ( string )( $i + 1 ) ) ) ); };

			$aResponsive = Gen::GetArrField( $ai, array( 'responsive' ), array() );
			if( !isset( $aResponsive[ 0 ] ) )
			{
				$aResponsive[ 0 ] = $ai;
				unset( $aResponsive[ 0 ][ 'responsive' ] );
			}
			...
			$dimMin = 0;
			foreach( $aResponsive as $dim => $aiR )
			{
				...
				if( Gen::GetArrField( $ai, array( 'adaptiveHeight' ), false ) )
				{
				...
				{
					$nDots = _ProcessCont_Cp_slick_GetDotsCount( array( 'slideCount' => $sld -> nSlides, 'slidesToShow' => $nShow, 'slidesToScroll' => Gen::GetArrField( $aiR, array( 'slidesToScroll' ), 1 ), 'infinite' => Gen::GetArrField( $ai, array( 'infinite' ), true ), 'centerMode' => Gen::GetArrField( $ai, array( 'centerMode' ), false ), 'asNavFor' => false ) );
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .' . implode( '.', $aDotsClass ) . ( $nDots ? ' > *:nth-child(n+' . ( $nDots + 1 ) . ')' : '' ) . ' {display:none;}' . "\n";
				}
			...
			if( Gen::GetArrField( $ai, array( 'arrows' ), true ) )
			{
				$itemPrevArrow = Gen::GetArrField( $ai, array( 'prevArrow' ), '<button class="slick-prev" aria-label="Previous" type="button">Previous</button>', '', false, false );
				if( is_string( $itemPrevArrow ) )
					$itemPrevArrow = HtmlNd::ParseAndImport( $doc, $itemPrevArrow );
				if( $itemPrevArrow )
				{
					HtmlNd::AddRemoveAttrClass( $itemPrevArrow, array( 'slick-arrow' ) );
					$sld -> itemSlides -> insertBefore( $itemPrevArrow, $sld -> itemSlides -> firstChild );
				}

				$itemNextArrow = Gen::GetArrField( $ai, array( 'nextArrow' ), '<button class="slick-next" aria-label="Next" type="button">Next</button>', '', false, false );
				if( is_string( $itemNextArrow ) )
					$itemNextArrow = HtmlNd::ParseAndImport( $doc, $itemNextArrow );
				if( $itemNextArrow )
				{
					HtmlNd::AddRemoveAttrClass( $itemNextArrow, array( 'slick-arrow' ) );
					$sld -> itemSlides -> appendChild( $itemNextArrow );
				}
			}

			if( Gen::GetArrField( $ai, array( 'dots' ), false ) )
			{

				_ProcessCont_Cp_slick_AddDots( $doc, $sld -> itemSlides, $aDotsClass, $sld -> nSlides, $cbCustomPaging, $sld );
			}
...
function _ProcessCont_Cp__jetCrsl( $ctx, &$ctxProcess, $settFrm, $doc, $xpath, array $aPrm )
{
	...
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," ' . $aPrm[ 'item' ] . ' ")][@data-settings]' ) as $item )
	{
		...
		$ai = array(
			...
			'customPaging' => function( $sld, $i ) { return( '<li tabindex="0"><span>' . ( $i + 1 ) . '</span></li>' ); },
		);
...
function _ProcessCont_Cp_wooPrdGallSld_2_3( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	foreach( HtmlNd::ChildrenAsArr( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," woocommerce-product-gallery ")][contains(concat(" ",normalize-space(@class)," ")," woo-product-gallery-slider ")]' ) ) as $item )
	{
		...
		$a[] = array(
			...
			'alignItems' => 'start',
			'responsive' => array(
				767 => array( 'vertical' => Gen::GetArrField( $aPrm, 'thumb_position_mobile' ) != 'bottom', 'slidesToShow' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_mobile_thumb_to_show' ), 'slidesToScroll' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_mobile_thumb_scroll_by' ) ),
				1025 => array( 'vertical' => Gen::GetArrField( $aPrm, 'thumb_v_tablet' ) != 'bottom', 'slidesToShow' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_tabs_thumb_to_show' ), 'slidesToScroll' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_tabs_thumb_scroll_by' ) ),
			),
		);
	}
...
function _ProcessCont_Cp_wooNasaPrdGall( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," nasa-single-product-slide ")]' ) as $item )
	{
		...
		$a[] = array( 'enable' => true, 'selCss' => '.nasa-single-product-slide .nasa-single-product-main-image', 'slidesToShow' => ( int )$item -> getAttribute( 'data-num_main' ), 'infinite' => $item -> getAttribute( 'data-infinite' ) != 'false', 'dots' => $item -> getAttribute( 'data-dots' ) != 'false' );
		$a[] = array( 'enable' => true, 'selCss' => '.nasa-single-product-slide .product-thumbnails', 'slidesToShow' => ( int )$item -> getAttribute( 'data-num_thumb' ), 'vertical' => true );
	}
...
function _ProcessCont_Cp_btCntSld( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," bt_bb_content_slider ")]/*[contains(concat(" ",normalize-space(@class)," ")," slick-slider ")][@data-slick]' ) as $item )
	{
		...
		$a[] = array( 'enable' => true, 'selCss' => '.' . $classid . '.bt_bb_content_slider .slick-slider', 'dots' => ($dataSett[ 'dots' ]??null), 'arrows' => ($dataSett[ 'arrows' ]??null) );
	}
