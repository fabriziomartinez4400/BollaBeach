...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.28.15' || $lastCheckPackage !== 'Ext' )
		{
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.28.15' ), '2.28.15' ) > 0 )
					{
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.28.15.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
		...
		if( ( $switch || version_compare( $verLast, '2.28.15' ) > 0 ) && !isset( $value -> response[ $plgBaseNameCur ] ) )
		{
		...
		if( isset( $value -> response[ $plgBaseNameCur ] ) )
		{
			...
			if( !$switch && version_compare( $verLast, '2.28.15' ) > 0 )
...
class PluginRe
{
	...
	static function _GetPhpLauncherEx( $sExtensionDir, $sPhpOs )
	{
		...
		if( 0 ) {}
			...
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/usr/local/php[\\d.]+/((?:ext|mod))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
